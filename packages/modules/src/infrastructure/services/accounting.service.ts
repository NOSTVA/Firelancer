...
export class AccountingService implements IAccountingService {
  ...
  async initializeAccountTransaction(details: {
    ...
    const createdTransaction = await transaction.create(async (tx) => {
      const account = await this._accountsRepository.getAccountByUserId(details.userId, tx);

      if (!account) {
        throw new Error("user does not have a valid account. please contact devs");
      }
      ...
      const created = await this._accountsRepository.createAccountTransaction(
        {
          id: transactionId,
    ...
    await this.settleAccountTransaction(createdTransaction.id);

    return createdTransaction;
  }

  async settleAccountTransaction(transactionId: string): Promise<void> {
    const transaction = getInjection("ITransaction");

    transaction.create(async (tx) => {
      const transaction = await this._accountsRepository.getAccountTransactionById(transactionId, tx);

      if (!transaction) {
        throw new Error("Account transaction settlement: transaction does not exist");
      }

      const isSettled = transaction.settledDate && transaction.balance;
      const isSettleable = !transaction.reviewDueDate || isBefore(transaction.reviewDueDate, new Date());

      if (isSettled) {
        throw new Error("Account transaction settlement: transaction already settled");
      }

      if (!isSettleable) {
        throw new Error("Account transaction settlement: transaction cannot be settled at the moment");
      }

      // The entry is neither settled nor deferred (meaning it’s eligible for settlement)
      // perform settlement:
      const latestSettledtransaction = await this._accountsRepository.getLatestSettledAccountTransaction(
      ...
      if (debit.isLessThan(0) || credit.isLessThan(0)) {
        throw new Error("Account transaction settlement: debits and credits cannot be negative values.");
      }

      if (debit.isEqualTo(0) && credit.isEqualTo(0)) {
        throw new Error("Account transaction settlement: debits and credits both cannot be 0");
      }

      if (balance.isLessThan(0)) {
        throw new Error("Account transaction settlement: balance caanot be negative.");
      }

      const settledEntry = await this._accountsRepository.updateAccountTransactionById(
        transaction.id,
        {
      ...
      if (!settledEntry) {
        throw new Error("Account transaction settlement: could not settle balance entry please try again");
      }
    });
  }
}
