...
export async function verifyEmailVerificationCodeUseCase(input: { userId: string; email: string; code: string }) {
  transaction.create(async (tx) => {
    const verificationCode = await emailVerificationCodeRepository.getVerificationCodeByUserId(input.userId, tx);

    if (!verificationCode || verificationCode.code !== input.code) {
      throw new UnauthorizedError("invalid verification code");
    }

    await emailVerificationCodeRepository.deleteVerificationCode(verificationCode.code, tx);
